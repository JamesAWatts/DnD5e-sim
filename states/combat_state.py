...
from core.combat.loot_manager import LootDropManager

from core.game_rules.constants import scale_x, scale_y, COLOR_WHITE, COLOR_GOLD, SCREEN_WIDTH, SCREEN_HEIGHT, FONT_PATH
...
class CombatState(BaseState):
    def __init__(self, game, font, player_data, enemy_data):
        ...
        self.mini_font = self.fonts['small']
        ...
        self.loot_mgr = LootDropManager()
        self.shake_mgr = ScreenShake()
        
        # Pre-allocate world surface for screen shake to avoid per-frame allocation
        self.world_surf = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
    ...
    def _handle_victory(self):
        ...
        CombatResolver.apply_combat_rewards(self.game, self.party, self.enemies, self.dialogue_mgr, is_flee=False)
    ...
    def _build_ability_registry(self):
        ...
        try:
            ...
            # Use generic get_data to ensure Web/Wasm compatibility and consistency
            spells_db = db.get_data('spells') or {}
            skills_db = db.get_data('skills') or {}
            items_db = db.get_data('consumables') or {}
            enemy_db = db.get_data('enemy_abilities') or {}
            ...
            for actor in self.party + self.enemies:
                ...
                if actor.get('is_enemy'):
                    actor_id = id(actor)
                    if actor_id not in self.ability_registry:
            ...
            print(f"\n[COMBAT] ERROR: Heavy data-loading failed in _build_ability_registry!")
            print(f"[COMBAT] REASON: {e}")
            print("[COMBAT] Loading empty placeholders to prevent canvas crash.\n")
            # Registry is already initialized as {}, so we just proceed
    ...
    def _register_actor_abilities(self, actor, spells_db, skills_db, items_db):
        ...
        # Pull consumables from Global Inventory for party members
        if not actor.get('is_enemy'):
            for item_name in self.game.inventory.get('consumable', {}):
                item_key = item_name.lower().replace(' ', '_')
                if item_key in items_db:
                    raw_data = items_db[item_key]
                    item_data = raw_data.copy()
                    item_data['_is_item'] = True
                    
                    # Map effect_type/value to fields resolve_action understands
                    e_type = item_data.get('effect_type')
                    val = item_data.get('value', 0)
                    if e_type == 'heal':
                        item_data['type'] = 'heal'
                        item_data['healing'] = val
                    elif e_type == 'restore_mana':
                        item_data['type'] = 'heal' # Still use heal flow for recovery
                        item_data['mana_gain'] = val
                    elif e_type == 'restore_stamina':
                        item_data['type'] = 'heal'
                        item_data['stamina_gain'] = val
                    elif e_type == 'buff_bonus' or e_type == 'temp_weapon_buff':
                        item_data['type'] = 'buff'
                        item_data['bonus_gain'] = val
                    elif e_type == 'buff_attacks':
                        item_data['type'] = 'buff'
                        item_data['attack_gain'] = val
                    elif e_type == 'on_hit_effect':
                        item_data['type'] = 'buff'
                        # These usually require extra logic, but we'll register them as buffs
                        
                    self.ability_registry[a_id][item_key] = item_data
        ...
        if actor.get('is_enemy'):
            for ab_id in actor.get('abilities', []):
                key = ab_id.lower().replace(' ', '_')
                if key in enemy_db:
                    self.ability_registry[a_id][key] = enemy_db[key]
    ...
    def apply_flash_effect(self, sprite, frames, flash_type):
        ...
        if flash_type == 'damage' and (frames // 6) % 2 == 1:
            try: return pygame.transform.grayscale(new_s)
                ...
                new_s.fill((100, 100, 100, 255), special_flags=pygame.BLEND_RGBA_MULT)
                return new_s
        return new_s
    ...
    def update(self, events, dt):
        ...
        # --- Phased Death Transition ---
        # If the 'defeated' message was just cleared, trigger the dissipation and loot fall
        if not self.dialogue_mgr.dialogue_box.current_message:
            for actor in self.party + self.enemies:
                if actor.get('is_dying'):
                    # 1. Dissipate the silhouette particles
                    for effect in self.vfx_mgr.active_effects:
                        if hasattr(effect, 'particles') and getattr(effect, 'is_static', False):
                            effect.trigger_dissipation()
                    
                    # 2. Start the loot drop animation
                    for drop in self.loot_mgr.active_drops:
                        if not drop.is_active:
                            drop.start_drop()
                    
                    # 3. Mark as fully done to prevent re-triggering
                    actor['is_dying'] = False
                    actor['death_sequence_complete'] = True
        ...
        # --- Delayed Victory Check ---
        if self.waiting_for_victory and not self.victory_triggered:
            # Only trigger victory if NO critical systems are busy:
            # 1. Dialogue manager is empty/finished
            # 2. VFX are done
            # 3. All loot drops have hit the ground
            critical_busy = (
                self.dialogue_mgr.is_busy() or 
                self.vfx_mgr.is_playing() or 
                any(not drop.is_grounded for drop in self.loot_mgr.active_drops)
            )
            
            if not critical_busy:
    ...
    def _handle_targeting_input(self, events):
        ...
        if result:
            if "target_grid" in result:
                ...
                # Remove Item from Inventory if applicable
                if self.pending_action_data.get('_is_item'):
                    item_name = self.pending_action_data.get('name')
                    from core.players.player_inventory import remove_item
                    remove_item(self.game.inventory, item_name, 'consumable', count=1)
                    # Re-sync ability registry to update counts if we were showing them?
                    # Actually registry doesn't store counts, but if count hits 0, it should be removed?
                    # The next time the menu is opened, it will be fresh from inventory.
                ...
                if has_dice:
                    ...
                    # Queue dice roll dialogue
                    if self.pending_action_data.get('type') == 'attack':
                        self.dialogue_mgr.queue_message("Rolling attack...")
                    elif self.pending_action_data.get('type') == 'save':
                        msg = "Rolling saves..." if len(target_list) > 1 else "Rolling save..."
    ...
    def _process_menu_action(self, action_dict):
        ...
        if action_type == "ATTACK":
            ...
            # Strip (xN) suffix from item names if present
            if "(" in ability_id and action_type == "USE_ITEM":
                import re
                ability_id = re.sub(r" \(x\d+\)$", "", ability_id)
    ...
    def _handle_run_attempt(self):
        ...
        if roll < run_chance:
            ...
            # Grant rewards for any enemies killed before fleeing
            CombatResolver.apply_combat_rewards(self.game, self.party, self.enemies, self.dialogue_mgr, is_flee=True)
    ...
    def _change_menu_state(self, new_state):
        ...
        if new_state == "MAIN":
            ...
            consumables = self.game.inventory.get('consumable', {})
            inv_names = [f"{name.replace('_', ' ').title()} (x{count})" for name, count in consumables.items()]
    ...
    def _handle_start_turn(self):
        ...
        self.current_actor['_swift_triggered'] = False
        ...
        if self.current_actor.get('current_hp', 1) <= 0:
            ...
            if self.current_actor in self.turn_queue:
                self.turn_queue.remove(self.current_actor)
            self.phase = 'DIALOGUE'
            return
        ...
        # --- 4. RESOURCE REGEN FOR AI & SUMMONS ---
        if is_ai:
        ...
        if self.current_actor in self.party:
            ...
            if self.current_actor.get('is_summon', False):
                ...
                # Identify opposing faction (for player summons, these are enemies)
                is_party_faction = (self.current_actor in self.party)
                opposing_list = self.enemies if is_party_faction else self.party
                living_opponents = [p for p in opposing_list if p.get('current_hp', 0) > 0]

                if living_opponents:
                    self.active_target_list = CombatAI.pick_target(self.current_actor, living_opponents, self.pending_action_data)
                ...
                if self.active_target_list:
                    ...
                    if self.pending_action_data.get('type') == 'attack':
                        ...
                        msg = "Rolling saves..." if len(self.active_target_list) > 1 else "Rolling save..."
                        self.dialogue_mgr.queue_message(msg)
    ...
    def _handle_end_turn(self):
        ...
        if self.turn_queue:
            # Look at the one who just finished
            actor = self.turn_queue.pop(0)
            # Only put them back if they are still alive
            if actor.get('is_alive', True) and actor.get('current_hp', 0) > 0:
                self.turn_queue.append(actor)
            else:
                print(f"[COMBAT] {actor.get('name')} was defeated and removed from the turn order.")

        # Ensure next actor is alive (Secondary cleanup)
        while self.turn_queue and not self.turn_queue[0].get('is_alive', True):
    ...
    def draw(self, screen):
        ...
        # (Already pre-allocated in self.world_surf, just filling)
        self.world_surf.fill((0, 0, 0))
        
        # 1. Background
        self.bg_mgr.draw(self.world_surf)
        
        # 2. Loot Drops (On the floor)
        self.loot_mgr.draw(self.world_surf)

        # 3. Active Sprites (Living actors)
        ...
        self.sprite_mgr.draw_sprites(self.world_surf, self)
        
        # 4. VFX (Drawn OVER sprites)
        self.vfx_mgr.draw(self.world_surf)
        
        # 5. Projectiles, Floats, Bars
        self.projectile_mgr.draw(self.world_surf)
        self.float_mgr.draw(self.world_surf)
        self.renderer.draw_entity_bars(self.world_surf)
        
        if self.phase == 'TARGETING':
            self.renderer.draw_targeting_cursor(self.world_surf)
        ...
        if getattr(self.game, 'god_mode', False) or (self.game.debug_overlay and self.game.debug_overlay.visible):
            self.renderer.draw_debug_placement(self.world_surf)

        # 2. Blit world_surf to screen with offset
        screen.blit(self.world_surf, (ox, oy))
    ...
    def _handle_enemy_ai(self):
        ...
        # 3. Target Selection
        # Identify opposing faction (Player summons target enemies, enemies target party)
        is_party_faction = (enemy in self.party)
        opposing_list = self.enemies if is_party_faction else self.party
        living_opponents = [p for p in opposing_list if p.get('current_hp', 0) > 0]
        
        if living_opponents:
            target = CombatAI.pick_target(enemy, living_opponents, self.pending_action_data)
            if target:
