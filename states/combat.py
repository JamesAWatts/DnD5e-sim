...
class CombatState(BaseState):
    ...
    def _initialize_combatants(self):
        ...
        for e in self.enemies:
            ...
            enemy_type = e.get('enemy_type', e.get('base_name', 'unknown'))
            variants = SpriteManager.get_enemy_variants(cat, enemy_type)
            e["sprite_filename"] = random.choice(variants)
        ...
        try:
            ...
            for s_type in summon_types:
                ...
                for i in range(summon_count):
                    ...
                    # Force directional folders based on faction
                    if not is_enemy:
                    ...
                    # Dynamic variant detection for summons in the forced directional folder
                    s_variants = SpriteManager.get_enemy_variants(category, s_type, folder_path=folder)
                    summon['sprite_filename'] = random.choice(s_variants)
    ...
    def process_loot(self, bonus=1.0):
        from core.players.player_inventory import add_item
    ...
    def exit_to_hub(self):
        ...
        if all(p['current_hp'] <= 0 for p in self.party):
            ...
            self.game.change_state(GameOverState(self.game, self.fonts))
        else:
