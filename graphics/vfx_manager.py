...
class SilhouetteDeathVFX:
    def __init__(self, sprite_surface, start_pos, effect_type="wind", is_static=False):
        self.particles = []
        ...
        # Configuration
        self.chunk_size = 4  # Size of each particle
        self.gravity = 0.5
        
        # 1. Generate Silhouette Particles
        width, height = sprite_surface.get_size()
        self.mid_x = start_pos[0] + width / 2
        self.mid_y = start_pos[1] + height / 2

        for x in range(0, width, self.chunk_size):
            for y in range(0, height, self.chunk_size):
                # Check the alpha value of the pixel
                color = sprite_surface.get_at((x, y))
                if color.a > 128:  # If it's mostly non-transparent
                    p_x = start_pos[0] + x
                    p_y = start_pos[1] + y
                    self.particles.append(DeathParticle(p_x, p_y, self.chunk_size))
                    
        # 2. Apply Initial Velocity based on Removal Effect
        self._apply_initial_forces()

    def trigger_dissipation(self):
        """Starts the animation phase if currently static."""
        self.is_static = False

    def _apply_initial_forces(self):
        for p in self.particles:
            ...
            elif self.effect_type == "cut_horizontal":
                if p.y < self.mid_y:
                    p.vx = random.uniform(-3, -1) # Top half slides left
                else:
                    p.vx = 0 # Bottom half stays still
                    
            elif self.effect_type == "cut_vertical":
                if p.x < self.mid_x:
                    p.vx = random.uniform(-4, -1) # Left half falls left
                else:
                    p.vx = random.uniform(1, 4)   # Right half falls right
    ...
    def update(self, dt):
        ...
        for p in self.particles:
            ...
            # Apply ongoing physics
            if self.effect_type == "wind":
                p.vx += random.uniform(0.1, 0.5)
                p.vy += random.uniform(-0.2, 0.2)
                p.alpha -= 5
                
            elif self.effect_type == "collapse":
                p.vy += self.gravity
                p.alpha -= 8
                
            elif self.effect_type in ["cut_horizontal", "cut_vertical"]:
                if self.timer > 300: # Delay before falling
                    p.vy += self.gravity
                p.alpha -= 6
    ...
    def draw(self, screen):
        for p in self.particles:
            if p.active:
                # Draw a solid black particle
                rect = pygame.Rect(p.x, p.y, p.size, p.size)
                # To support alpha on rects in Pygame, we draw to a temporary surface
                s = pygame.Surface((p.size, p.size), pygame.SRCALPHA)
                s.fill((0, 0, 0, max(0, int(p.alpha))))
...
class VFXManager:
    ...
    def play_death_vfx(self, target, effect_type=None, is_static=False):
        """Creates a silhouette shatter effect for a dying unit."""
        # 1. Get the sprite and position
        from graphics.sprite_manager import SpriteManager
        sprite = SpriteManager.get_enemy_sprite(target)
        
        # Handle sprite animation list/dict
        if isinstance(sprite, list) and len(sprite) > 0:
            sprite = sprite[0]
        elif isinstance(sprite, dict):
            sprite = sprite.get('idle', [None])[0]

        if not sprite or not isinstance(sprite, pygame.Surface):
            return

        # 2. Get position
        v_rect = target.get('_visual_rect')
        pos = (v_rect.x, v_rect.y) if v_rect else target.get('screen_pos', (0, 0))
        
        # 3. Randomize effect type if not provided
        if not effect_type:
            effect_type = random.choice(["wind", "collapse", "explode", "cut_horizontal", "cut_vertical"])

        # 4. Create and add
        effect = SilhouetteDeathVFX(sprite, pos, effect_type=effect_type, is_static=is_static)
        self.active_effects.append(effect)
