...
class TransitionManager:
    ...
    def _make_grayscale(self, surface):
        ...
        try:
            ...
            # Fallback using surfarray (Faster than PixelArray for full screen)
            try:
                import pygame.surfarray as surfarray
                import numpy as np
                # Get the pixel array
                pixels = surfarray.array3d(surface)
                # Apply standard grayscale weights (R, G, B)
                grayscale_pixels = np.dot(pixels[...,:3], [0.299, 0.587, 0.114])
                # Stack to get RGB again
                grayscale_pixels = np.stack((grayscale_pixels,)*3, axis=-1).astype(np.uint8)
                return surfarray.make_surface(grayscale_pixels)
            except:
                # Final fallback using PixelArray
                gs = surface.copy()
                pixels = pygame.PixelArray(gs)
                for x in range(gs.get_width()):
                    for y in range(gs.get_height()):
                        color = surface.get_at((x, y))
                        avg = int(0.299 * color.r + 0.587 * color.g + 0.114 * color.b)
