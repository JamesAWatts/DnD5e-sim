...
class SpriteManager:
    ...
    def draw_sprites(self, screen, state):
        ...
        for summon in summons_sorted:
            if summon.get('current_hp', 0) <= 0: continue
    ...
    def draw_bars(self, screen, state):
        ...
        res_font = getattr(state, 'mini_font', pygame.font.Font(None, scale_y(18)))
    ...
    def get_enemy_sprite(enemy_data, size=None):
        # 1. Check for Direct Path Override or Summon Routing
        folder = enemy_data.get('sprite_folder')
        filename = enemy_data.get('sprite') or enemy_data.get('sprite_filename')
        
        if enemy_data.get('is_summon'):
            if enemy_data.get('is_enemy'):
        ...
        # 2. Identify who we are and where we live
        base_name = enemy_data.get('base_name', 'unknown').lower()
        category = enemy_data.get('category', 'general')

        # 3. Try to find the file variants in our new nested map
        try:
            category_map = SpriteManager._enemy_mapping.get(category, {})
            variants = category_map.get(base_name, [f"{base_name}.png"])
    ...
    @staticmethod
    def get_player_sprite(class_name, size=(192, 192), entity=None):
        ...
        if entity and entity.get('is_summon'):
            ...
            # Standard non-summon player routing
        ...
        try:
            if not os.path.exists(sprite_path):
                # Fallbacks for standard players
                if not (entity and entity.get('is_summon')):
                    ...
                    raise FileNotFoundError(f"No summon sprite found for {filename}")
