import pygame
import random
from core.game_rules.constants import FONT_PATH
...
class DevTools:
    ...
    def apply_dev_action(action, game):
        ...
        if action == "1,000 HP":
            ...
            fonts = getattr(game.state, 'fonts', None)
            game.change_state(LevelUpState(game, fonts, is_dev_mode=True))
            ...
            game.player = None
            fonts = getattr(game.state, 'fonts', None)
            game.change_state(ClassSelectState(game, fonts))
