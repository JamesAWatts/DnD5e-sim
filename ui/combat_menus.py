...
class AbilityDescriptionBox:
    ...
    def draw(self, screen, baked_ability):
        ...
        res_type = baked_ability.get('resource', 'sp').upper()
        ...
        # Name
        draw_text_outlined(screen, name, self.font, COLOR_GOLD, rect.x + scale_x(15), rect.y + scale_y(15))

        # Cost and Type
        info_text = f"{cost} {res_type} | {a_type}"
        small_font = pygame.font.Font(None, scale_y(24))
        ...
        draw_text_outlined(screen, info_text, small_font, cost_color, rect.x + scale_x(15), rect.y + scale_y(45))
        ...
        for word in words:
            test_line = line + word + " "
            if small_font.size(test_line)[0] < rect.width - scale_x(30):
                line = test_line
            else:
                draw_text_outlined(screen, line, small_font, COLOR_WHITE, rect.x + scale_x(15), desc_y)
                desc_y += scale_y(22)
                line = word + " "
        draw_text_outlined(screen, line, small_font, COLOR_WHITE, rect.x + scale_x(15), desc_y)
...
class CombatMenuManager:
    ...
    def open_main_menu(self, actor=None):
        ...
        opts = col1 + col2
        # Anchor to bottom-left area
        self.main_menu = Menu(opts, self.font, header=f"{actor['name']}", pos=(self.BASE_X, self.BASE_Y), columns=[col1, col2])
